      ...
      <main className="min-h-[941px] min-w-0 flex-1 bg-[#F4F5F7]">
        <div className="mx-auto w-full max-w-[672px] px-[32px] pt-[40px]">
          <header className="h-[54px] w-full">
            <h1 className="font-chillax text-[24px] font-bold text-[#0E1726]">
              Attendance
            </h1>
          ...
          <div className="mt-[24px] grid grid-cols-3 gap-[12px]">
            <OverviewCard
              value={stats.total_registered.toString()}
              label="Expected"
              color="text-[#0E1726]"
            />

            <OverviewCard
              value={stats.total_checked_in.toString()}
              label="Checked In"
              color="text-[#1C2E5A]"
            />
          ...
          <div className="mt-6 w-full rounded-[24px] border border-[#1C2E5A1A] bg-white p-6 shadow-sm">
            {loading ? (
              <div className="py-20 text-center text-[#6B7590]">
                Loading attendance data...
              </div>
            ) : participants.length === 0 ? (
              <div className="flex flex-col items-center py-10">
                <UsersRound className="mb-4 h-12 w-12 text-[#A8BBCE]" />

                <h2 className="font-chillax text-lg font-bold text-[#0E1726]">
                  No check-ins yet
                </h2>

                <Link
                  href="/checkin"
                  className="mt-6 flex items-center gap-2 rounded-xl bg-[#162E55] px-6 py-3 text-white"
                >
                ...
                {participants.map((p) => (
                  ...
                  >
                      ...
                      <div className="text-right">
                        ...
                        {p.check_in_time && (
                          <p className="mt-1 text-xs text-[#6B7590]">
                            {new Date(
                              p.check_in_time
                            ).toLocaleTimeString(
                              [],
...
function OverviewCard({
  ...
  return (
    <div className="flex h-[73px] flex-col items-center justify-center rounded-[16px] bg-[#EEF1F5] p-[12px]">
      <p
        className={`font-chillax text-[24px] font-bold ${color}`}
      >
        {value}
      </p>

      <p className="mt-1 font-inter text-[10px] text-[#6B7590]">
        {label}
      </p>
    </div>
  );
