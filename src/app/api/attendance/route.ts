...
const filters = z.object({
  ...
  status: z.enum(['pending', 'checked_in']).optional(),
  page: z.coerce.number().int().min(1).max(100000).default(1),
});

export const GET = api(async request => {
  if (!(await getAuthorizedStaff())) {
    throw new HttpError(403, 'Coordination Team access is required.');
  }

  const parsed = filters.safeParse(Object.fromEntries(new URL(request.url).searchParams));
  if (!parsed.success) {
    throw new HttpError(400, 'Invalid attendance filters.');
  }

  const input = parsed.data;
  const { data, error } = await db().rpc('attendance_report', {
    event_day: input.date || eventDate(),
    search_name: input.name,
    search_org: input.organization,
    filter_role: input.role || null,
